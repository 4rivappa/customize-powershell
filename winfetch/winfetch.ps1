    ...
    if ($image) {
        ...
        if ($logo -eq "Windows 11") {
            ...
            @(
                ...
                "${e}[${t};34m            ``/++++/+++++++:"
                "${e}[${t};34m           ``/++++++++++++++:"
                "${e}[${t};34m          ``/+++ooooooooooooo/``"
                "${e}[${t};34m         ./ooosssso++osssssso+``"
                "${e}[${t};34m        .oossssso-````````/ossssss+``"
                "${e}[${t};34m       -osssssso.      :ssssssso."
                "${e}[${t};34m      :osssssss/        osssso+++."
                "${e}[${t};34m     /ossssssss/        +ssssooo/-"
                "${e}[${t};34m   ``/ossssso+/:-        -:/+osssso+-"
                "${e}[${t};34m  ``+sso+:-``                 ``.-/+oso:"
                "${e}[${t};34m ``++:.                           ``-/+/"
                "${e}[${t};34m .``                                 ``/"
            )
...
foreach ($item in $config) {
    if (Test-Path Function:"info_$item") {
        ...
        $info = @{ title = "$e[31mfunction 'info_$item' not found$e[0m" }
    }
